...
| Tenant isolation | Every organization route resolves `Membership(organizationId, userId)`. Non-members get 404 (no existence oracle); ids are validated. Roles are per membership. Permission checks are centralized in `config/roles.js`. |
